...
export class Payout extends FincraCore {
    ...
    */
    public async walletToWalletTransfer(data: WalletToWalletTransferDto) {
        try {
            const request = this.getBaseUrl();
            const response = await request.post(`/disbursements/payouts/wallets`, data);
            return response.data;
        } catch (error:any) {
            console.error(error);
            throw new BaseError({message: error.response.data})
    ...
    */
    public async fetchPayout(reference: string) {
        try {
            const request = this.getBaseUrl();
            const response = await request.get(`/disbursements/payouts/reference/${reference}`);
            return response.data;
        } catch (error:any) {
            console.error(error);
            throw new BaseError({message: error.response.data})
    ...
    */
    public async fetchCustomerPayout(creference: string) {
        try {
            const request = this.getBaseUrl();
            const response = await request.get(`/disbursements/payouts/customer-reference/${creference}`);
            return response.data;
        } catch (error:any) {
            console.error(error);
            throw new BaseError({message: error.response.data})
